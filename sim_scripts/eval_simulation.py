...
python sim_scripts/eval_simulation.py \
...
# 2025-10-2
python examples/SO-100/m_eval_lerobot.py \
    --robot.type=my_custom_robot \
    --robot.host=127.0.0.1 \
    --robot.observation_port=65435 \
    --robot.articulation_port=65433 \
    --robot.id=my_awesome_follower_arm \
    --robot.cameras="{ wrist: {type: opencv, index_or_path: 9, width: 640, height: 480, fps: 30}, front: {type: opencv, index_or_path: 15, width: 640, height: 480, fps: 30}}" 
    --lang_instruction="Grab the black pen, the blue pen and the rubber, then place them into the white bowl."
...
python examples/SO-100/m_eval_lerobot.py \
    ...
    --robot.id=my_awesome_follower_arm \
    --robot.cameras="{ wrist: {type: opencv, index_or_path: 9, width: 640, height: 480, fps: 30}, front: {type: opencv, index_or_path: 15, width: 640, height: 480, fps: 30}}" 
    --lang_instruction="Pick up the black pen and place it into the bowl"
...
import numpy as np
# from ....lerobot.src.lerobot.cameras.opencv.configuration_opencv import (  # noqa: F401
#     OpenCVCameraConfig,
# )
...
class Gr00tRobotInferenceClient:
    ...
    def get_action(self, observation_dict, lang: str):
        ...
        print("robot_state_keys", self.robot_state_keys)
...
@dataclass
class EvalConfig:
    robot: MyCustomRobotConfig  # the robot to use
